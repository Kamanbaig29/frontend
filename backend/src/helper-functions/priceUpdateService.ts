...
//     console.log(
//       `🔄 Starting price update for ${walletTokens.length} tokens for user ${userPublicKey.toBase58()}`
//     );

//     for (let i = 0; i < walletTokens.length; i += BATCH_SIZE) {
//       const batch = walletTokens.slice(i, i + BATCH_SIZE);
//       await Promise.all(
//         batch.map(async (walletToken) => {
//           try {
...
//           } catch (error) {
//             console.error(
//               `❌ Error updating price for mint ${walletToken.mint}:`,
//               error
//             );
//           }
//         })
...
//     console.log(
//       `⏱️ Price update cycle completed in ${totalTime.toFixed(2)} seconds for user ${userPublicKey.toBase58()}`
//     );
//   } catch (error) {
//     const endTime = Date.now();
//     const totalTime = (endTime - startTime) / 1000;
//     console.error(
//       `❌ Error in price update cycle after ${totalTime.toFixed(2)} seconds:`,
//       error
//     );
//   }
// }
...
// ) {
//   console.log(
//     `🚀 Price update service started for user ${userPublicKey.toBase58()} with ${interval / 1000 / 60} minute interval`
//   );
//   let stopped = false;
...
//     console.log(
//       `🔄 Starting price update cycle at ${new Date().toLocaleTimeString()} for user ${userPublicKey.toBase58()}`
//     );

//     try {
//       await updatePrices(connection, userPublicKey);
//       console.log(
//         `✅ Price update cycle completed at ${new Date().toLocaleTimeString()} for user ${userPublicKey.toBase58()}`
//       );
//     } catch (error) {
...
//     console.log(
//       `⏹️ Price update service stopped for user ${userPublicKey.toBase58()}`
//     );
//   };
// }
